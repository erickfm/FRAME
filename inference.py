#!/usr/bin/env python3
# inference.py  –  FRAME bot (strict-mapping edition)
#
# • Streams live libmelee frames → dataset-compatible tensors
# • Mirrors every transformation used to build training Parquets
# • Hard-fails on unknown enum values or missing columns
# ---------------------------------------------------------------------------
...
# ── CLI / logging ───────────────────────────────────────────────────────────
parser = argparse.ArgumentParser(description="Realtime FRAME bot (strict)")
parser.add_argument("--debug", action="store_true",
                    help="Verbose sanity checks")
...
from dataset   import (
    MeleeFrameDataset,   # ← canonical class name
    TOKEN_SPEC,
    encode_cstick_dir,
)
...
model = FramePredictor(cfg).to(DEVICE, non_blocking=True)
...
def check_tensor_dict(tdict: Dict[str, torch.Tensor], where: str) -> None:
    ...
    for k, v in tdict.items():
        ...
        if v.device != DEVICE:
            log.debug("%s tensor on %s (expected %s)", k, v.device, DEVICE)
...
# 3)  dataframe utils
# ════════════════════════════════════════════════════════════════════════════
def encode_cstick_dir_df(df: pd.DataFrame, prefix: str,
                         dead: float = 0.15) -> None:
    if f"{prefix}_c_x" in df.columns and f"{prefix}_c_y" in df.columns:
        encode_cstick_dir(df, prefix, dead)

def _map_cat(col: str, raw: Any) -> int:
    """
    Convert raw Slippi enum value/name → contiguous dataset ID.

    • '', None, -1, or NaN  → sentinel raw 0  (matches training)
    • Else: unknown values raise KeyError.
    """
    emap = _spec._enum(col)                       # {raw_value: 0..N-1}

    # ---------- normalise sentinel "empty" cases ------------------------
    if raw in ("", None, "None", -1) or (isinstance(raw, float) and np.isnan(raw)):
        raw = 0

    # ---------- resolve enum names to numeric ---------------------------
    if isinstance(raw, str) and raw:
        try:
            if col == "stage":
                raw = melee.enums.Stage[raw].value
            elif col.endswith("_character"):
                raw = melee.enums.Character[raw].value
            elif col.endswith("_action"):
                raw = melee.enums.Action[raw].value
            elif col.endswith("_type"):
                raw = melee.enums.ProjectileType[raw].value
        except KeyError as err:
            raise KeyError(f"{col}: unknown enum name '{raw}'") from err

    try:
        raw_int = int(raw)
    except (ValueError, TypeError) as err:
        raise KeyError(f"{col}: non-numeric value '{raw}'") from err

    if raw_int not in emap:
        raise KeyError(f"{col}: raw value {raw_int} missing from enum map")

    return emap[raw_int]

def rows_to_state_seq(rows: List[Dict[str, Any]]) -> Dict[str, torch.Tensor]:
    """
    Convert a rolling window of raw libmelee row dicts → tensor dict (B=1)
    that matches the training-time dataset spec.
    """
    # ------------------------------------------------------------------ #
    # 1. Build DataFrame and do early, *string-safe* feature engineering #
    # ------------------------------------------------------------------ #
    df = pd.DataFrame(rows).drop(columns=["startAt"], errors="ignore")

    # --- bucket C-stick directions (operates on floats) ----------------
    for p in ("self", "opp", "self_nana", "opp_nana"):
        encode_cstick_dir_df(df, p)

    # --- distance between players -------------------------------------
    if {"self_pos_x", "opp_pos_x"} <= set(df.columns):
        df["distance"] = np.hypot(
            df["self_pos_x"] - df["opp_pos_x"],
            df["self_pos_y"] - df["opp_pos_y"],
        ).astype("float32")

    # --- facing flags (these cols are numeric already) -----------------
    for col in ("self_facing", "opp_facing",
                "self_nana_facing", "opp_nana_facing"):
        if col in df.columns:
            df[col] = (df[col] > 0).astype("float32")

    # ------------------------------------------------------------------ #
    # 2. Basic NaN / Inf hygiene                                         #
    # ------------------------------------------------------------------ #
    ...
    # ── guarantee Nana C-stick bucket cols exist (neutral = 0) ─────────
    if {"self_nana_c_dir", "opp_nana_c_dir"} - set(df.columns):
        df = df.assign(
            self_nana_c_dir=df.get("self_nana_c_dir", 0),
            opp_nana_c_dir=df.get("opp_nana_c_dir", 0),
        )

    # ------------------------------------------------------------------ #
    # 3. Ensure *all* categorical columns exist, then strict-map them     #
    # ------------------------------------------------------------------ #
    missing = [c for c in _categorical_cols if c not in df.columns]
    if missing:
    ...
    if missing_num_cols:
        df = pd.concat(
            [df, pd.DataFrame(missing_num_cols, index=df.index)],
            axis=1,
        )

    # ── collapses all column blocks into one; warning gone ──────────────
    df = df.copy()

    # ------------------------------------------------------------------ #
    # 6. Convert each spec → tensor, stack a batch dimension, return     #
    # ------------------------------------------------------------------ #
...
# ════════════════════════════════════════════════════════════════════════════
# 5)  Controller output helpers
# ════════════════════════════════════════════════════════════════════════════
C_DIR_TO_FLOAT = {0: (0.5, 0.5), 1: (0.5, 1.0), 2: (0.5, 0.0),
                  3: (0.0, 0.5), 4: (1.0, 0.5)}

def _safe(val: float, default: float = 0.5) -> float:
    return default if not math.isfinite(val) else min(max(val, 0.0), 1.0)
...
if __name__ == "__main__":
    DOLPHIN_APP = ("/Users/erick/Library/Application Support/"
                   "Slippi Launcher/netplay/Slippi Dolphin.app")
    ISO_PATH    = "/Users/erick/Downloads/melee.iso"

    console = melee.Console(path=DOLPHIN_APP, slippi_address="127.0.0.1",
                            fullscreen=False)
    ...
    while True:
        ...
        # ---------- build row ----------
        row: Dict[str, Any] = {
            "stage":    gs.stage.name if gs.stage else "",
            "frame":    gs.frame,
        ...
        if len(rows) == ROLL_WIN:
            ...
            if start_wait > 0:
                ...
                log.info(
                    "[%d] MAIN=(%.2f,%.2f) C=(%.2f,%.2f) L=%.2f R=%.2f BTN=%s",
