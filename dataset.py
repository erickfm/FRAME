...
class MeleeFrameDataset(Dataset):
    def __init__(self, parquet_dir:str, seq_len:int=60, delay:int=1):
        ...
        self.enums = {
            ...
            "_subtype": {i: i for i in range(42)},  # arbitrary; update as needed
    ...
    def __getitem__(self, idx:int):
        ...
        # --- NUMERIC COERCION: fix None/NaN/inf per column -------------
        float_cols = [
            c
        ...
        for c in float_cols:
            df[c] = (
                pd.to_numeric(df[c], errors="coerce")  # None / text → NaN
                .fillna(0.0)                           # NaN         → 0.0
                .replace([np.inf, -np.inf], 0.0)       # inf         → 0.0
                .astype("float32")
            )
