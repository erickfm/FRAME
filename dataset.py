...
# -----------------------------------------------------------------------------
# Produces tensors used by frame_encoder.py:
...
# ──────────────────────────────────────────────────────────────────────
# 0. Enum maps  (swap in your real dictionaries)
# ──────────────────────────────────────────────────────────────────────
...
class TensorSpec:
    ...
    shape: Tuple[int,...]
    token: str                 # unused by code but handy for docs
...
def flag_cols(ent:str): return [f"{ent}_{f}" for f in PLAYER_FLAGS]
...
class MeleeFrameDataset(Dataset):
    def __init__(
        ...
        # enum maps (identity for unmapped suffixes)
        self.enums = {
            "stage":         STAGE_MAP,
    ...
    def _enum(self, col: str) -> Dict[int, int]:
        if col == "stage":
            return self.enums["stage"]
        for suff, mp in self.enums.items():
            if suff != "stage" and col.endswith(suff):
                return mp
        # unseen suffix → identity map
        return {v: v for v in range(int(self._current_df[col].max()) + 1)}
    ...
    # -----------------------------------------------------------------
    def _clamp_nonfinite(self, df: pd.DataFrame) -> None:
        """Replace any NaN/±Inf in float columns with zero (in-place)."""
        num_cols = df.select_dtypes(include=["float32", "float64"]).columns
        if not len(num_cols):
            return
        arr = df[num_cols].to_numpy(dtype=np.float32, copy=False)
        mask = ~np.isfinite(arr)
        if mask.any():
            if self.debug:
                bad_counts = mask.sum(axis=0)
                offenders = {c: int(n) for c, n in zip(num_cols, bad_counts) if n}
                print("🟠 non-finite clamped:", offenders)
            arr[mask] = 0.0
            df.loc[:, num_cols] = arr
